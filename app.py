    ...
    for sosok, limit in [(0, 5), (1, 5)]:
        ...
        for page in range(1, limit + 1):
            ...
            try:
                ...
                # Try lxml, then fallback to html.parser
                try:
...
def main():
    ...
    if df.empty:
        filtered_df = pd.DataFrame(columns=['번호', '종목코드', '종목명', '산업카테고리', 'DeltaPER', '추정 PER', '현재 PER', '추정 ROE', '부채비율', '이익성장률'])
    else:
        if show_all_500:
            filtered_df = df.copy()
        else:
            # Only dropna if columns exist to avoid KeyError
            actual_cols = [c for c in required_cols if c in df.columns]
            if actual_cols:
                filtered_df = df.dropna(subset=actual_cols)
            else:
                filtered_df = df.copy()
    ...
    if apply_filters and not filtered_df.empty:
        ...
        if '추정 PER' in filtered_df.columns: cond &= (filtered_df['추정 PER'] <= max_fwd_per)
        if '추정 ROE' in filtered_df.columns: cond &= (filtered_df['추정 ROE'] >= min_roe)
        if '부채비율' in filtered_df.columns: cond &= (filtered_df['부채비율'] <= max_debt)
        if '시가총액(억)' in filtered_df.columns: cond &= (filtered_df['시가총액(억)'] >= min_mcap)
        
        filtered_df = filtered_df[cond]
        st.caption(f"필터 적용 후 {len(filtered_df)}행")
    
    if not filtered_df.empty and 'DeltaPER' in filtered_df.columns:
